...
def link_tbl_to_traces(
    tbl,
    linear_traces, circ_traces,
    ufovcells_linear, ufovcells_circ,
    return_combined=False
):
    """
    Augment tbl with stable indices that point back to your original trace matrices.

    Inputs
    ------
    tbl : pandas.DataFrame
        Must have columns: 'pool' ('linear'|'circular'), 'cell_id' (int), 'trace_id' (int, per-cell index)
    linear_traces : (M_lin, L, 6) float
    circ_traces   : (M_circ, L, 6) float
    ufovcells_linear : (M_lin,) array of ints, cell_id for each linear trace row
    ufovcells_circ   : (M_circ,) array of ints, cell_id for each circular trace row
    return_combined  : bool, if True also returns a combined matrix and adds 'combined_row'

    Returns
    -------
    tbl_out : pandas.DataFrame (copy of tbl with new columns)
        - 'global_row' : int index into the respective original matrix (linear or circular)
        - 'matrix_name': 'linear' or 'circular' (just echoes pool for clarity)
        - 'combined_row' : int index into combined matrix [linear; circular] (only if return_combined=True)
    matrices : dict
        Always includes:
          - 'linear_traces': linear_traces (passed through)
          - 'circular_traces': circ_traces (passed through)
        If return_combined=True also includes:
          - 'combined_traces': np.concatenate([linear_traces, circ_traces], axis=0)
    """
    import pandas as pd

    tbl_out = tbl.copy()

    ufovcells_linear = np.asarray(ufovcells_linear)
    ufovcells_circ   = np.asarray(ufovcells_circ)

    # Precompute, for each cell, the global index list (preserving original order)
    lin_cell_to_globs = {}
    for cid in np.unique(ufovcells_linear):
        lin_cell_to_globs[int(cid)] = np.where(ufovcells_linear == cid)[0]

    circ_cell_to_globs = {}
    for cid in np.unique(ufovcells_circ):
        circ_cell_to_globs[int(cid)] = np.where(ufovcells_circ == cid)[0]

    # Prepare output columns
    global_rows   = np.full(len(tbl_out), -1, dtype=int)
    matrix_name   = np.empty(len(tbl_out), dtype=object)
    combined_rows = np.full(len(tbl_out), -1, dtype=int) if return_combined else None

    # Combined offset (linear first)
    M_lin  = linear_traces.shape[0]
    # M_circ = circ_traces.shape[0]  # not needed explicitly

    # Fill indices row-by-row
    for i, (pool, cid, tlocal) in enumerate(zip(tbl_out["pool"], tbl_out["cell_id"], tbl_out["trace_id"])):
        pool = str(pool)
        cid  = int(cid)
        tlocal = int(tlocal)

        if pool == "linear":
            glob_list = lin_cell_to_globs.get(cid, None)
            if glob_list is None or tlocal < 0 or tlocal >= len(glob_list):
                # leave as -1
                matrix_name[i] = "linear"
                continue
            g = int(glob_list[tlocal])  # global row into linear_traces
            global_rows[i] = g
            matrix_name[i] = "linear"
            if return_combined:
                combined_rows[i] = g  # linear block comes first

        elif pool == "circular":
            glob_list = circ_cell_to_globs.get(cid, None)
            if glob_list is None or tlocal < 0 or tlocal >= len(glob_list):
                matrix_name[i] = "circular"
                continue
            g = int(glob_list[tlocal])  # global row into circ_traces
            global_rows[i] = g
            matrix_name[i] = "circular"
            if return_combined:
                combined_rows[i] = M_lin + g  # offset by linear size
    ...
    tbl_out["global_row"] = global_rows
    tbl_out["matrix_name"] = matrix_name
    if return_combined:
        tbl_out["combined_row"] = combined_rows

    matrices = {
        "linear_traces": linear_traces,
        "circular_traces": circ_traces,
    }
    if return_combined:
        matrices["combined_traces"] = np.concatenate([linear_traces, circ_traces], axis=0)

    return tbl_out, matrices

def run_single_gene_intron_colocalization(iQ, dic_thh,
                                            linear_traces, circ_traces):
    all_tables_SEP = []
    
    ufovcells_linear = np.nanmean(linear_traces[:,:, -1], axis = 1)
    ufovcells_circ = np.nanmean(circ_traces[:,:, -1], axis = 1)
    
    for ifovcell in tqdm(np.unique(ufovcells_linear)):
        # slice per cell
        linear_traces_cell   = linear_traces[ufovcells_linear == ifovcell]
        circular_traces_cell = circ_traces[ufovcells_circ == ifovcell]

        # anchors for this cell
        X_SEP_cell = np.load(os.path.join(intron_analysis_folder, f'cell{int(ifovcell)}--iQ{iQ}--th_h{dic_thh[iQ]}--XFb.npy'))

        # assignment (you already do this)
        out = assign_traces_to_XQ_mean_gate_dual(
            linear_traces=linear_traces_cell,
            circular_traces=circular_traces_cell,
            XQ_pixels=X_SEP_cell,
            pix_size=pix_size,
            Rmax_um=2.0
        )

        # neighbor density matrices (you already compute these)
        linear_NN_mat, circular_NN_mat = compute_neighbor_density_matrices_per_cell(
            linear_traces_cell, circular_traces_cell, N=3
        )

        # build per-cell table
        rec, df = build_trace_assignment_table_per_cell(
            out, X_SEP_cell, linear_NN_mat, circular_NN_mat,
            brightness_for_unassigned=0.0,  # or np.nan
            return_pandas=True
        )
        all_tables_SEP.append(df if df is not None else rec)

    tbl_SEP = pd.concat(all_tables_SEP, ignore_index=True)
    
    ## link to mat
    tbl_linked, mats = link_tbl_to_traces(
    tbl=tbl_SEP,
    linear_traces=linear_traces,
    circ_traces=circ_traces,
    ufovcells_linear=ufovcells_linear,
    ufovcells_circ=ufovcells_circ,
    return_combined=False  # set False if you don't want a combined matrix
)
    return tbl_linked
...
_iq_pat = re.compile(r"iQ(\d+)")
def _parse_iq_from_filename(path: str) -> int:
    m = _iq_pat.search(str(path))
    if not m:
        raise ValueError(f"Could not find iQ in file name: {path}")
...
def summarize_bursting_props_and_p(
    save_iQ_tbl_list,
    uCols, medianH_C,           # per-color stats: len(uCols) x 2 -> [median, std]
    dic_hyb_color,              # dict: iQ -> color (e.g., {19:0, 37:1, ...})
    th_zscore_dict,             # dict: color -> z threshold
    nn_thresh=1.0,
    ...
    stats_col_df = pd.DataFrame(medianH_C, index=pd.Index(uCols, name="col"),
                                columns=["median", "std"]).astype(float)
    def get_gene_name(filename):
        return os.path.basename(filename).split('__')[1].split('_introns')[0]
    def _safe_gene(path: str) -> str:
        try:
            return get_gene_name(path)  # uses your function if available
        except Exception:
    ...
    for file in save_iQ_tbl_list:
        ...
        if not {"avg_NN_dist", brightness_col, pool_col}.issubset(tbl.columns):
            rows.append({"gene": gene, "p_chr7": np.nan, "p_ecDNA": np.nan,
                         "pval": np.nan, "table": None})
            continue

        df = tbl.loc[tbl["avg_NN_dist"] >= nn_thresh, [brightness_col, pool_col]].copy()
        df = df[df[pool_col].isin(["linear", "circular"])]
        if df.empty:
            rows.append({"gene": gene, "p_chr7": np.nan, "p_ecDNA": np.nan,
                         "pval": np.nan, "table": None})
            continue

        # iQ -> color for this file
        ...
        # z-scores for all traces in this file
        h = df[brightness_col].to_numpy(float)
        mask = np.isfinite(h)
        df = df.loc[mask]
        z  = (df[brightness_col].to_numpy(float) - med) / std
        is_burst = z >= th

        # counts per pool
        tmp = pd.DataFrame({pool_col: df[pool_col].to_numpy(), "burst": is_burst})
        counts = tmp.groupby(pool_col)["burst"].value_counts().unstack(fill_value=0)
        # ensure both pools/cols exist
        for pl in ["linear", "circular"]:
            if pl not in counts.index:
                counts.loc[pl] = {False: 0, True: 0}
        cnt_lin_nb = int(counts.loc["linear"  , False])
        cnt_lin_b  = int(counts.loc["linear"  , True])
        cnt_cir_nb = int(counts.loc["circular", False])
        cnt_cir_b  = int(counts.loc["circular", True])

        # proportions
        p_lin = cnt_lin_b / max(cnt_lin_nb + cnt_lin_b, 1)
        p_cir = cnt_cir_b / max(cnt_cir_nb + cnt_cir_b, 1)

        # 2x2 for Fisher: rows=chr7/ecDNA, cols=non/burst
        table = np.array([[cnt_lin_nb, cnt_lin_b],
                          [cnt_cir_nb, cnt_cir_b]], dtype=int)
        # need at least one count in each row
        ...
        rows.append({"gene": gene, "p_chr7": p_lin, "p_ecDNA": p_cir,
                     "pval": pval, "table": table})

    return pd.DataFrame(rows)
import numpy as np
from tqdm import tqdm

def get_medianH_for_XF(
    XF,
    subsample=50,
    seed=None,
    show_progress=True,
    group_by="R",  # "R" to group by rounds (uRs), "col" to group by colors (uCols)
):
    """
    Compute per-group (R or col) median and std of brightness from XF.

    XF columns assumed (from the end, as in your code):
        -5: brightness (e.g., H or logH)
        -4: col              (int, e.g. 0/1/2)
        -3: R (round index)  (int, iR)
        -2: ground_truth     (unused)
        -1: cell id          (int)
    ...
    XF : np.ndarray
        Array of shape (N, ?).
    subsample : int, default 50
        Number of unique cells to sample (without replacement). If fewer exist,
        all available cells are used.
    seed : int or None, default None
        Random seed for reproducible subsampling.
    show_progress : bool, default True
        Show a tqdm bar.
    group_by : {"R","col"}, default "R"
        Whether to aggregate by rounds (uRs) or by colors (uCols).

    Returns
    -------
    medianH : np.ndarray, shape (n_groups, 2)
        Columns are [median, std] for each group in `ugroups`.
    ugroups : np.ndarray
        Sorted unique group values: either uRs (ints) or uCols (0/1/2).
    """
    ...
    cells = XF[:, CELL_COL].astype(int)
    ucells = np.unique(cells)
    if subsample > len(ucells):
        subsample = len(ucells)
    ucells_sub = rng.choice(ucells, size=subsample, replace=False) if subsample > 0 else ucells

    # choose grouping
    if group_by.lower() == "r":
        group_vec = XF[:, R_COL].astype(int)
        ugroups = np.unique(group_vec)
        desc = "Computing medianH by R"
    elif group_by.lower() == "col":
        group_vec = XF[:, COL_COL].astype(int)
        ugroups = np.unique(group_vec)
        desc = "Computing medianH by col"
    else:
        raise ValueError("group_by must be 'R' or 'col'")

    medianH_list = []
    iterator = tqdm(ugroups, desc=desc, disable=not show_progress)

    for g in iterator:
        is_g = (group_vec == g)
        XF_g = XF[is_g]

        # collect brightness across the subsampled cells for this group
        vals = []
        for ucell in ucells_sub:
            mask_cell = (XF_g[:, CELL_COL].astype(int) == ucell)
            if np.any(mask_cell):
                h = XF_g[mask_cell][:, BRIGHT_COL]
                h = h[np.isfinite(h)]
                if h.size:
                    vals.append(h)

        if len(vals) == 0:
            medianH_list.append([np.nan, np.nan])
            continue
